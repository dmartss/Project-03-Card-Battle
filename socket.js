...
module.exports = server => {
  ...
  io.on("connection", socket => {
    ...
    socket.on("join game", data => {
      ...
      let notification = {
        message: { message: `${data.username} ready for battle! :` }
      };
    ...
    const fightFunction = room => {
      ...
      if (
        playerOne.userSelection.type === "King" &&
        playerTwo.userSelection.type === "Queen"
      ) {
        hpTwo = hpTwo - 10;
      } else if (
        playerTwo.userSelection.type === "King" &&
        playerOne.userSelection.type === "Queen"
      ) {
        hpOne = hpOne - 10;
      } else if (defenseOne < attackTwo && attackOne > defenseTwo) {
      ...
      if (
        ...
        rounds[room]++;
        io.sockets.in(room).emit("round over", {
          playerData: publicPlayers[room],
          round: rounds[room]
        });
      }
    ...
    socket.on("no rematch", data => {
      ...
      let notification = {
        message: { message: `${data.username} is bowing out!` }
      };
      players[data.room] = [];
    ...
    socket.on("rematch", data => {
      ...
      if (playersReady[data.room].length === 2) {
        console.log(
          `resetting game for ${playersReady[data.room][0]} and ${
            playersReady[data.room][1]
          }!`
        );
        let notification = {
          message: {
            message: `${playersReady[data.room][0]} and ${
              playersReady[data.room][1]
            } are game for a rematch!`
          }
        };
        rounds[data.room] = 1;
        io.sockets
          .in(data.room)
          .emit("reset game", { round: rounds[data.room], winner: null });
        io.sockets.in(data.room).emit("load players", publicPlayers[data.room]);
        io.sockets.in(data.room).emit("receive message", notification);
        messages[data.room].push(notification.message);
        playersReady[data.room] = [];
    ...
    socket.on("leave room", data => {
      ...
      socket.broadcast
        .to(data.room)
        .emit("load players", publicPlayers[data.room]);
    ...
    socket.on("disconnect", () => {
      ...
      for (let room in users) {
        users[room] = users[room].filter(user => {
          if (user.socketId === socket.id) {
            ...
            console.log(
              `removed ${disconnectedUsername} from room ${disconnectedUserRoom}`
            );
          }
      ...
      if (disconnectedUsername) {
        let notification = {
          message: { message: `${disconnectedUsername} left the room! :` }
        };
        socket.broadcast
          .to(disconnectedUserRoom)
          .emit("load players", publicPlayers[disconnectedUserRoom]);
        socket.broadcast
          .to(disconnectedUserRoom)
          .emit("load users", users[disconnectedUserRoom]);
        socket.broadcast
          .to(disconnectedUserRoom)
        ...
        lobbyRef
          .child("users")
          .child(disconnectedUserRoom)
          .set(users[disconnectedUserRoom].length);
        lobbyRef
          .child("players")
          .child(disconnectedUserRoom)
          .set(players[disconnectedUserRoom].length);
      }
    });
  });
};
